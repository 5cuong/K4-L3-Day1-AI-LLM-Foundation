...
def format_comparison_table(results: list[dict]) -> str:
    ...
    rows = []
    for item in results:
        prompt = str(item.get("prompt", ""))[:40]
        gpt4o_response = str(item.get("gpt4o_response", ""))[:40]
        mini_response = str(item.get("mini_response", ""))[:40]
        gpt4o_latency = f"{float(item.get('gpt4o_latency', 0.0)):.3f}s"
        mini_latency = f"{float(item.get('mini_latency', 0.0)):.3f}s"
        rows.append(
            [prompt, gpt4o_response, mini_response, gpt4o_latency, mini_latency]
        )
